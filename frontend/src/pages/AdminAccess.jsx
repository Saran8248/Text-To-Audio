...
const AdminAccess = ({ currentUser, onUpdateUser }) => {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((user) =>
      `${user.name} ${user.email} ${user.role} ${user.accessStatus}`
        .toLowerCase()
        .includes(query),
    );
  }, [searchQuery, users]);
  ...
  const handleCreateUser = async (role) => {
    ...
    toast.success(
      role === "admin" ? "Admin added with access" : "User added with access",
    );
  };
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <p className="text-gray-400">
          Approve users, add admins, and control who can log in.
        </p>
      </motion.div>
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">
                Create an approved account directly.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-400 mt-1">
                {users.length} account records in temporary storage.
              </p>
            </div>
                  ...
                  >
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span
                          className={`px-2.5 py-1 rounded-full text-xs font-medium ${user.role === "admin" ? "bg-emerald-500/20 text-emerald-300" : "bg-blue-500/20 text-blue-300"}`}
                        >
                          {user.role}
                        </span>
                        <span
                          className={`px-2.5 py-1 rounded-full text-xs font-medium ${user.accessStatus === "approved" ? "bg-green-500/20 text-green-300" : "bg-yellow-500/20 text-yellow-300"}`}
                        >
                          {user.accessStatus}
                        </span>
                      </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {user.accessStatus !== "approved" && (
                        <button
                          onClick={() =>
                            handleAccessUpdate(user.id, "approved")
                          }
